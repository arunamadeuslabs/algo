...
DASHBOARD_SCRIPT="$ALGO_DIR/dashboard_server.py"
...
# 1. Mon-Fri at 09:10 IST — Start algos (launcher includes dashboard auto-start)
...
# 3. On reboot — Start dashboard server + static file server
CRON_REBOOT="@reboot cd $ALGO_DIR && $PYTHON $DASHBOARD_SCRIPT --port 8050 --no-browser >> $DASHBOARD_LOG 2>&1 & sleep 2 && python3 -m http.server 8080 > /dev/null 2>&1 &"
...
# Start the dashboard server now (if not already running)
if ! pgrep -f "dashboard_server.py" > /dev/null; then
    cd $ALGO_DIR && nohup $PYTHON $DASHBOARD_SCRIPT --port 8050 --no-browser >> $DASHBOARD_LOG 2>&1 &
    echo "  Live dashboard started on port 8050"
else
    echo "  Live dashboard already running on port 8050"
fi

# Start static file server for tradebook.html/dashboard.html
...
echo "     Strategies: EMA Crossover, Sapphire, Momentum, Iron Condor"
...
echo "     Log:     $REPORT_LOG"
echo ""
echo "  3. Live Dashboard: Auto-start on reboot"
echo "     URL: http://$(curl -s ifconfig.me):8050"
echo ""
echo "  4. Static Pages:   Auto-start on reboot"
echo "     Tradebook: http://$(curl -s ifconfig.me):8080/tradebook.html"
echo "     Report:    http://$(curl -s ifconfig.me):8080/dashboard.html"
echo ""
echo "  Current crontab:"
crontab -l | grep -E "launcher|daily_report|http.server|dashboard_server"
echo ""
