...
# Start live dashboard (port 8050)
nohup $PYTHON $DASHBOARD --port 8050 --no-browser >> dashboard.log 2>&1 &
DASH_PID=$!
ok "Live dashboard started (PID $DASH_PID, port 8050)"

# Start static file server for tradebook.html/dashboard.html (port 8080)
nohup python3 -m http.server 8080 >> static_server.log 2>&1 &
STATIC_PID=$!
ok "Static server started (PID $STATIC_PID, port 8080)"
...
CRON_REBOOT="@reboot sleep 30 && cd $ALGO_DIR && export \$(grep -v '^\#' $ALGO_DIR/.env | xargs) && $PYTHON $DASHBOARD --port 8050 --no-browser >> $DASHBOARD_LOG 2>&1 & sleep 2 && cd $ALGO_DIR && python3 -m http.server 8080 >> $ALGO_DIR/static_server.log 2>&1 &"
